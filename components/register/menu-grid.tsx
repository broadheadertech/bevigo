...
type MenuGridProps = {
 ...
 onCustomizeTap?: (item: LocationItem) => void;
};

export function MenuGrid({ categories, items, onItemTap, onCustomizeTap }: MenuGridProps) {
 ...
 className={`px-4 py-2 rounded-2xl text-sm font-medium whitespace-nowrap transition-colors min-h-[40px] ${
 activeTab === tab.id
 ...
 <div
 key={item._id}
 className="rounded-xl text-left transition-all duration-100 overflow-hidden flex flex-col relative group"
 style={{
 ...
 className="text-left flex flex-col w-full active:scale-95"
 style={{ color: 'inherit' }}
 >
 {item.imageUrl ? (
