    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div className="bg-white dark:bg-stone-900 rounded-2xl shadow-xl w-full max-w-sm mx-4 p-8 text-center">
          ...
          <button
            onClick={onClose}
            className="mt-4 px-4 py-2 text-sm font-medium text-stone-600 hover:text-stone-900"
          >
            Close
          </button>
        </div>
      ...
      <div className="bg-white dark:bg-stone-900 rounded-2xl shadow-xl w-full max-w-sm mx-4 overflow-hidden print:shadow-none print:rounded-none print:max-w-none print:mx-0">
              ...
              <div key={idx}>
                ...
                {item.modifiers.map(
                  (mod: ReceiptModifier, modIdx: number) => (
                    <div
                      key={modIdx}
                      className="flex justify-between text-xs text-stone-500 pl-3"
                    >
                      <span>+ {mod.name}</span>
                      {mod.priceAdj > 0 && (
                        <span className="ml-2 flex-shrink-0">
        ...
        <div className="px-6 pb-6 flex gap-3 print:hidden">
          <button
            ...
            className="flex-1 py-2.5 bg-stone-100 text-stone-700 font-medium rounded-lg hover:bg-stone-200 active:bg-stone-300 transition-colors text-sm"
          >
            Print
          </button>
          <button
            onClick={onClose}
            className="flex-1 py-2.5 bg-stone-800 text-white font-medium rounded-lg hover:bg-stone-900 active:bg-stone-950 transition-colors text-sm"
          >
            Close
          </button>
        </div>
