...
export default function BillingPage() {
  ...
  if (session.role !== "owner") {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-stone-500">Only owners can access billing settings.</p>
      </div>
    );
  }
    ...
    <div>
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-8">
        ...
        <Link
          ...
          className="self-start md:self-auto px-4 py-2.5 bg-stone-900 text-white text-sm font-semibold rounded-xl hover:bg-stone-800 transition-colors shadow-sm"
        >
          Change Plan
        </Link>
      </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center h-48">
          <p className="text-stone-400">Loading billing data...</p>
        </div>
      ) : (
          ...
          <div className="bg-white rounded-2xl border border-stone-200/60 shadow-sm p-6">
            ...
            {subscription && plan ? (
              ...
              <div className="space-y-3">
                ...
                <Link
                  href="/billing/upgrade"
                  className="inline-block px-4 py-2 bg-stone-900 text-white text-sm font-semibold rounded-xl hover:bg-stone-800 transition-colors"
                >
                  Choose a Plan
                </Link>
              </div>
