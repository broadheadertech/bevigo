...
function PaymentTypesTab({ data }: { data: PaymentTypeRow[] | undefined }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Table */}
      <div
        className="rounded-2xl border shadow-sm overflow-hidden overflow-x-auto"
        style={{
          backgroundColor: "var(--card)",
          borderColor: "var(--border-color)",
        }}
          ...
          {data.map((row: TimePeriodRow) => {
            ...
            return (
              <div key={row.period} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-6 rounded-lg overflow-hidden" style={{ backgroundColor: "var(--muted)" }}>
                  <div
                    className={`h-full rounded-lg transition-all ${isPeak ? "bg-amber-600" : "bg-amber-300"}`}
